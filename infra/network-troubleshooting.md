...
![nc 결과: 규칙 추가 전 5555 / 규칙 추가 후 5555 / 대조군 5556](../images/nacl_result.png)

위에서부터 **규칙 추가 전 5555 → 규칙 추가 후 5555 → 규칙 추가 후 5556** 순서로 실행한 결과입니다.

| 출발지 포트 | 규칙 추가 전 | 규칙 추가 후 |
| --- | --- | --- |
| `5555` | `Connected`, 0.016초 | `TIMEOUT.`, **10.021초** |
| `5556` (대조군) | - | `Connected`, 0.016초 |

같은 규칙 아래에서 5556은 정상이고 5555만 막혔습니다. **응답의 목적지 포트가 5555인 연결만** 골라서 막혔다는 뜻입니다.

### 요청은 DB에 도착했을까: `SYN-RECV`로 확인

웹 EC2에서 보면 실험 1-1(보안 그룹 차단)과 똑같이 `TIMEOUT`입니다. 요청이 DB에 닿지 못한 것인지, 닿았는데 응답이 막힌 것인지는 **DB EC2 쪽의 TCP 연결 상태**로 구분합니다.

TCP는 연결 전에 패킷 3개를 주고받습니다(3-way handshake).

```
웹 EC2 (클라이언트)                     DB EC2 (서버 :3306)
        │ ── ① SYN ──────────────────▶  │  "연결하고 싶어요"
        │ ◀─────────────── ② SYN-ACK ── │  "좋아요"            ← 실험 2에서는 여기서 NACL에 막힘
        │ ── ③ ACK ──────────────────▶  │  "확인했어요"
```

| 상태 | 의미 |
| --- | --- |
| `SYN-RECV` | 서버가 ①을 받고 ②를 보냈지만, ③을 아직 못 받은 상태 |
| `ESTABLISHED` | ③까지 끝나 연결 완료 |

DB EC2에서 소켓 상태를 계속 출력해 두고, 웹 EC2에서 규칙 추가 후 5555로 접속했습니다.

```bash
# [DB EC2] 0.1초마다 5555 관련 소켓 상태 출력
watch -n 0.1 "ss -tan | grep 5555"
```

![DB EC2의 SYN-RECV 상태](../images/nacl_syn_recv.png)

```
SYN-RECV  0  0  [::ffff:10.0.2.10]:3306  [::ffff:10.0.1.35]:5555
```

- DB EC2(`ip-10-0-2-10`)가 웹 EC2(`10.0.1.35`)의 5555에서 온 **요청(①)을 받았고**, 응답(②)을 보낸 뒤 확인(③)을 기다리는 중입니다.
- 즉 **요청은 도착했고, DB의 응답이 DB 서브넷을 나가다 막혔습니다.** 실험 1-1의 보안 그룹 차단 때는 요청이 DB에 닿지 않으므로 이런 상태가 남지 않습니다.
- `[::ffff:10.0.2.10]`은 IPv4 주소를 IPv6 형식으로 표시한 것입니다(IPv4-mapped IPv6). MariaDB가 IPv4와 IPv6를 함께 받는 소켓으로 3306을 열고 있어서 이렇게 보이며, 실제 통신은 IPv4입니다.
- DB는 확인(③)이 오지 않으면 응답(②)을 몇 차례 다시 보내 보다가 포기하므로, `nc`가 10초 만에 끝난 뒤에도 이 상태가 1분 가까이 남아 있습니다.
...
| 규칙의 포트 칸 | 목적지 포트 (인바운드·아웃바운드 모두) | 목적지 포트 (인바운드·아웃바운드 모두) |
| 응답용 임시 포트 규칙 | 필요 없음 | 반드시 필요 (1024-65535) |
...
3. **보안 그룹과 NACL 모두 규칙의 포트 칸은 항상 목적지 포트입니다.** 인바운드든 아웃바운드든 같고, 방향에 따라 바뀌는 건 IP 칸(인바운드는 소스, 아웃바운드는 대상)뿐입니다. 출발지 포트로 거르는 칸은 없습니다.
   - 실험 2에서 두 서브넷이 같은 NACL을 쓰는데도 웹 EC2의 요청이 막히지 않은 이유가 이것입니다. 요청(`10.0.1.35:5555 → 10.0.2.10:3306`)의 5555는 **출발지** 포트라서 아웃바운드 규칙 `90`(목적지 포트 5555)과 비교되지 않습니다. 규칙에 걸린 건 목적지 포트가 5555인 DB의 응답뿐입니다.
4. **보안 그룹은 응답 패킷을 규칙으로 검사하지 않습니다.** 허용된 요청의 응답은 연결 추적(connection tracking)으로 자동 통과합니다. 그래서 같은 "목적지 5555 거부"를 보안 그룹으로 만들었다면 실험 2는 아무 영향이 없었을 것입니다.
   - 같은 이유로 보안 그룹 규칙을 지워도 **이미 맺어진 연결은 바로 끊기지 않을 수 있습니다.** 실험 1에서는 `nc`가 매번 새로 연결했기 때문에 바로 `TIMEOUT`이 났습니다.
   - 보안 그룹 아웃바운드는 **요청**에는 적용됩니다. `web-sg` 아웃바운드를 기본값(모두 허용)보다 좁힌다면 `3306 → db-sg`는 허용해 둬야 합니다.
5. **NACL을 쓸 때는 응답이 오가는 임시 포트(1024-65535)도 열어 둬야 합니다.** 응답의 목적지 포트는 서비스 포트(80, 3306)가 아니라 요청한 쪽의 임시 포트이기 때문입니다. 새 NACL은 기본이 전부 거부라서, DB 서브넷에 인바운드 3306만 허용하면 요청은 들어가도 응답이 나가지 못해 연결되지 않습니다. 아웃바운드와 상대 서브넷 인바운드에 임시 포트를 함께 허용해야 합니다.
